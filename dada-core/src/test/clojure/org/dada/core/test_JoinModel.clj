...
(deftest rhs-first
  ...
  (is (= (get-mutable join-model) [(map-new) [(map-new)(map-new)] nil nil nil]))

  ;; insert rhs (lhs does not exist)
  (def jules-0 [:jules 0 "Julian Gosnell"])
  (insert account-model jules-0)
  (is (= (get-mutable join-model) [(map-new)
				   [(map-new)
				    (map-new2 :jules (RHSEntry. jules-0 [#{} #{}]))]
				   nil nil nil]))
  ...
  (let [new-datum [0 0 txn-0 jules-0 jane-0 gbp-0]]
    (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 0 txn-0 [[gbp-0] [jane-0 jules-0]] new-datum))
				     [(map-new2 :gbp   (RHSEntry. gbp-0   [#{0}]))
				      (map-new2 :jane  (RHSEntry. jane-0  [#{0} #{}])
  ...
  (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 0 txn-0 [[gbp-0] [jane-0 jules-0]]
							  [0 0 txn-0 jules-0 jane-0 gbp-0]))
   				   [(map-new2 :usd   (RHSEntry. usd-0   [#{}])
  ...
  (let [old-datum [0 0 txn-0 jules-0 jane-0 gbp-0]
    ...
    (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 1 txn-1 [[usd-0] [jules-0 jane-0]] new-datum))
				     [(map-new2 :usd   (RHSEntry. usd-0   [#{0}])
						:gbp   (RHSEntry. gbp-0   [#{}]))
				      (map-new2 :jane  (RHSEntry. jane-0  [#{} #{0}])
						:jules (RHSEntry. jules-0 [#{0} #{}]))
				      ]
  ...
  (let [old-datum [0 1 txn-1 jane-0 jules-0 usd-0]
    ...
    (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 2 txn-1 [[usd-0] [jules-1 jane-0]] new-datum))
				     [(map-new2 :usd   (RHSEntry. usd-0   [#{0}])
						:gbp   (RHSEntry. gbp-0   [#{}]))
				      (map-new2 :jane  (RHSEntry. jane-0  [#{} #{0}])
  ...
  (let [old-datum [0 2 txn-1 jane-0 jules-1 usd-0]
    ...
    (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 3 txn-1 [[usd-0] [jules-1 jane-1]] new-datum))
				     [(map-new2 :usd   (RHSEntry. usd-0   [#{0}])
						:gbp   (RHSEntry. gbp-0   [#{}]))
				      (map-new2 :jane  (RHSEntry. jane-1  [#{} #{0}])
						:jules (RHSEntry. jules-1 [#{0} #{}]))
				      ]
				     nil [(Update. old-datum new-datum)] nil])))
  ...
  (let [old-datum [0 3 txn-1 jane-1 jules-1 usd-0]
    ...
    (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 4 txn-2 [[usd-0] [jules-1 jane-1]] new-datum))
				     [(map-new2 :usd   (RHSEntry. usd-0   [#{0}])
						:gbp   (RHSEntry. gbp-0   [#{}]))
				      (map-new2 :jane  (RHSEntry. jane-1  [#{} #{0}])
						:jules (RHSEntry. jules-1 [#{0} #{}]))
  ...
  (let [old-datum [0 4 txn-2 jane-1 jules-1 usd-0]
    ...
    (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 5 txn-3 [[gbp-0] [jules-1 jane-1]] new-datum))
				     [(map-new2 :usd   (RHSEntry. usd-0   [#{}])
						:gbp   (RHSEntry. gbp-0   [#{0}]))
				      (map-new2 :jane  (RHSEntry. jane-1  [#{} #{0}])
						:jules (RHSEntry. jules-1 [#{0} #{}]))
...
(deftest lhs-first
  ...
  (def ^ModelView join-model (JoinModel.
			      "Join"
			      (seq-metadata 3)
			      transaction-model
			      {3 account-model
  ...
  (let [old-datum nil
    ...
    (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 0 txn-0 [[nil] [nil nil]] new-datum))
				     [(map-new2 :gbp   (RHSEntry. nil [#{0}]))
				      (map-new2 :jane  (RHSEntry. nil [#{0} #{}])
  ...
  (let [old-datum [0 0 txn-0 nil nil nil]
    ...
    (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 1 txn-1 [[nil] [nil nil]] new-datum))
				     [(map-new2 :usd   (RHSEntry. nil [#{0}])
						:gbp   (RHSEntry. nil [#{}]))
				      (map-new2 :jane  (RHSEntry. nil [#{} #{0}])
						:jules (RHSEntry. nil [#{0} #{}]))]
				     nil [(Update. old-datum new-datum)] nil])))
  ...
  (let [old-datum [0 1 txn-1 nil nil nil]
    ...
    (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 2 txn-1 [[nil] [jules-0 nil]] new-datum))
				     [(map-new2 :usd   (RHSEntry. nil [#{0}])
						:gbp   (RHSEntry. nil [#{}]))
				      (map-new2 :jane  (RHSEntry. nil [#{} #{0}])
						:jules (RHSEntry. jules-0 [#{0} #{}]))]
  ...
  (let [old-datum [0 2 txn-1 nil jules-0 nil]
    ...
    (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 3 txn-1 [[nil] [jules-0 jane-0]] new-datum))
				     [(map-new2 :usd   (RHSEntry. nil [#{0}])
						:gbp   (RHSEntry. nil [#{}]))
				      (map-new2 :jane  (RHSEntry. jane-0  [#{} #{0}])
						:jules (RHSEntry. jules-0 [#{0} #{}]))]
  ...
  (def gbp-0 [:gbp 0 "British Pounds"])
  (insert currency-model gbp-0)
  (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 3 txn-1 [[nil] [jules-0 jane-0]]
  ...
  (let [old-datum [0 3 txn-1 jane-0 jules-0 nil]
    ...
    (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 4 txn-1 [[usd-0] [jules-0 jane-0]] new-datum))
				     [(map-new2 :usd   (RHSEntry. usd-0   [#{0}])
						:gbp   (RHSEntry. gbp-0   [#{}]))
				      (map-new2 :jane  (RHSEntry. jane-0  [#{} #{0}])
						:jules (RHSEntry. jules-0 [#{0} #{}]))]
  ...
  (let [old-datum [0 4 txn-1 jane-0 jules-0 usd-0]]
    (is (= (get-mutable join-model)
	   [(map-new2 0 (LHSEntry. false 4 txn-1 [[nil] [nil nil]] old-datum))
	    [(map-new2 :usd   (RHSEntry. usd-0   [#{}])
		       :gbp   (RHSEntry. gbp-0   [#{}]))
	     (map-new2 :jules (RHSEntry. jules-0 [#{} #{}])
  ...
  (let [new-datum [0 5 txn-2 jane-0 jules-0 usd-0]]
    (is (= (get-mutable join-model) [(map-new2 0 (LHSEntry. true 5 txn-2 [[usd-0] [jules-0 jane-0]] new-datum))
				     [(map-new2 :usd   (RHSEntry. usd-0 [#{0}])
						:gbp   (RHSEntry. gbp-0 [#{}]))
				      (map-new2 :jules (RHSEntry. jules-0 [#{0} #{}])
  ...
  (let [old-datum [0 5 txn-2 jane-0 jules-0 usd-0]
    ...
    (is (= (get-mutable join-model)
  	   [(map-new2 0 (LHSEntry. false 5 txn-3 [[nil] [nil nil]] old-datum))
  	    [(map-new2 :usd   (RHSEntry. usd-0   [#{}])
		       :gbp   (RHSEntry. gbp-0   [#{}]))
  	     (map-new2 :jules (RHSEntry. jules-0 [#{} #{}])
		       :jane  (RHSEntry. jane-0  [#{} #{}]))]
